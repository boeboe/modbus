...
func main() {
	...
	for opIdx := 0; opIdx < len(runList); opIdx++ {
		...
		case readBools:
			...
			if o.isCoil {
				res, err = client.ReadCoils(context.Background(), currentUnitId, o.addr, o.quantity+1)
			} else {
				res, err = client.ReadDiscreteInputs(context.Background(), currentUnitId, o.addr, o.quantity+1)
			}
			if err != nil {
		...
		case readUint16, readInt16:
			...
			if o.isHoldingReg {
				res, err = client.ReadRegisters(context.Background(), currentUnitId, o.addr, o.quantity+1, modbus.HoldingRegister)
			} else {
				res, err = client.ReadRegisters(context.Background(), currentUnitId, o.addr, o.quantity+1, modbus.InputRegister)
			}
			if err != nil {
		...
		case writeCoil:
			err = client.WriteCoil(context.Background(), currentUnitId, o.addr, o.coil)
			if err != nil {
		...
		case writeUint16:
			err = client.WriteRegister(context.Background(), currentUnitId, o.addr, o.u16)
			if err != nil {
		...
		case scanBools:
			performBoolScan(context.Background(), client, currentUnitId, o.isCoil)

		case scanRegisters:
			performRegisterScan(context.Background(), client, currentUnitId, o.isHoldingReg)

		case scanUnitId:
			performUnitIdScan(context.Background(), client)

		case ping:
			performPing(context.Background(), client, currentUnitId, o.quantity, o.duration)
...
func performUnitIdScan(ctx context.Context, client *modbus.ModbusClient) {
	var err error
	var countOk uint
	var countErr uint
